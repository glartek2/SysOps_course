    ...
    if (listener_pid < 0)
        ...
        while (!should_close) {
            ...
            switch (receive_message.type) {
                case MESSAGE_TEXT:
                    printf("Otrzymano od użytkownika : %d wiadomość: %s\n", receive_message.identifier, receive_message.text);
                    break;
                case IDENTIFIER:
                    printf("Otrzymano id od servera: %d\n", receive_message.identifier);
                    write(to_parent_pipe[1], &receive_message.identifier, sizeof(receive_message.identifier));
                    break;
                default:
                    printf("Nieznany typ wiadomości: %d", receive_message.type);
                    break;
            }
        }
        printf("Wyjście z pętli otrzymywania wiadomości\n");
        exit(0);
    } else {
        ...
        while (!should_close) {
            ...
            if (attributes.mq_curmsgs >= attributes.mq_maxmsg) {
                printf("Serwer jest pełny, proszę czekać\n");
                continue;
            }
        ...
        printf("Wyjście z pętli wysyłającej\n");
